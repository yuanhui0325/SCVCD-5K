...
#    val_loader = DataLoader(dataset=val_dataset, shuffle=False, num_workers=0, batch_size=1, pin_memory=True)
#    net.eval()
#
#    sum_rd = 0.0
#    sum_psnr = 0.0
...
#                net(input_image, ref_image, quant_noise_feature, quant_noise_z, quant_noise_mv)
#
#            mse_loss = torch.mean(mse_loss)
#            warploss = torch.mean(warploss)
#            interloss = torch.mean(interloss)
#            bpp = torch.mean(bpp)
#
#            warp_weight = 0.1 if global_step < 500000 else 0
#            distortion = mse_loss + warp_weight * (warploss + interloss)
#            rd_loss = train_lambda * distortion + bpp
#
#            # metrics
#            psnr = 10 * (torch.log(1.0 / mse_loss) / np.log(10)) if mse_loss > 0 else torch.tensor(100.0).cuda()
#
#            sum_rd += rd_loss.item()
#            sum_psnr += psnr.item()
#            sum_bpp += bpp.item()
...
#
#    net.train()
#    return sum_rd / max(cnt, 1), sum_psnr / max(cnt, 1), sum_bpp / max(cnt, 1)
#
...
#    event_dir = os.environ.get("EVENT_DIR", "./events")
#    tb_logger = SummaryWriter(event_dir)
##    train_dataset = DataSet("data/vimeo_septuplet/test.txt")
#    train_dataset = DataSet("/data/lichaofei/data/SCVCD-NEW/train/train.txt")
#    val_dataset = DataSet("/data/lichaofei/data/SCVCD-NEW/val/val.txt", augment=False)
...
##        adjust_learning_rate(optimizer, global_step)
##        if global_step > tot_step:
##            save_model(model, global_step, name="latest.model")
##            break
##
##        global_step = train(epoch, global_step)
##
...
##        save_model(model, global_step, name="latest.model")
##
##        # 2) 定期验证并保存 best（用 test_step 控制频率）
##        if global_step % test_step == 0:
##            val_rd, val_psnr, val_bpp = validate(global_step)
##            tb_logger.add_scalar('val/rd_loss', val_rd, global_step)
##            tb_logger.add_scalar('val/psnr', val_psnr, global_step)
##            tb_logger.add_scalar('val/bpp', val_bpp, global_step)
##
##            
##            if val_rd < best_val_loss:
##                best_val_loss = val_rd
##                save_model(model, global_step, name="best.model")
#    for epoch in range(stepoch, tot_epoch):
#        adjust_learning_rate(optimizer, global_step)
#
#        if global_step > tot_step:
...
#        val_rd, val_psnr, val_bpp = validate(global_step)
#
#        # 建议打印/写日志，否则 run.out 看不到验证结果
#        log = f"[VAL] epoch={epoch} step={global_step} rd_loss={val_rd:.6f} psnr={val_psnr:.4f} bpp={val_bpp:.6f}"
#        print(log)
#        logger.info(log)
#
#        tb_logger.add_scalar('val/rd_loss', val_rd, global_step)
...
#        tb_logger.add_scalar('val/bpp', val_bpp, global_step)
#
#        # 3) 保存 best（按 val_rd 最小）
#        if val_rd < best_val_loss:
#            best_val_loss = val_rd
#            save_model(model, global_step, name="best.model")
#            print(f"[VAL] best updated: {best_val_loss:.6f}")
#            logger.info(f"[VAL] best updated: {best_val_loss:.6f}")
#
#
#测试
...
#parser.add_argument('--config', dest='config', required=True,
#        help = 'hyperparameter of Reid in json format')
#parser.add_argument('--testscvcd7', action='store_true')
#parser.add_argument('--scvcd_seq_root', type=str, default='/data/lichaofei/data/SCVCD-NEW/test/sequences')
#parser.add_argument('--scvcd_rec_root', type=str, default='/data/lichaofei/data/SCVCD-NEW/test/DVC_rec_I')
#parser.add_argument('--scvcd_list', type=str, default='/data/lichaofei/data/SCVCD-NEW/test/DVC_test')
#parser.add_argument('--scvcd_refdir', type=str, default='H265L20')
#def parse_config(config):
#    config = json.load(open(args.config))
#    global tot_epoch, tot_step, test_step, base_lr, cur_lr, lr_decay, decay_interval, train_lambda, ref_i_dir
#    if 'tot_epoch' in config:
...
#    global train_dataset, test_dataset
#    if args.testscvcd7:
#        from dataset import SCVCD_DVC_Test7
#        test_dataset = SCVCD_DVC_Test7(
#            seq_root=args.scvcd_seq_root,
#            rec_root=args.scvcd_rec_root,
#            filelist=args.scvcd_list,
#            refdir=args.scvcd_refdir
#        )
#        print('testing SCVCD 7-frame dataset...')
#        testuvg(0, testfull=False)   # 直接复用原来的测试循环
#        exit(0)
#
#    tb_logger = SummaryWriter('./events')
#    train_dataset = DataSet("data/vimeo_septuplet/test.txt")
...
parser.add_argument('--scvcd_list', type=str, default='/data/lichaofei/data/SCVCD-NEW/test/DVC_test')
parser.add_argument('--scvcd_refdir', type=str, default='H265L20')
parser.add_argument('--oracle_i', action='store_true')
def parse_config(config):
    config = json.load(open(args.config))
    global tot_epoch, tot_step, test_step, base_lr, cur_lr, lr_decay, decay_interval, train_lambda, ref_i_dir
    if 'tot_epoch' in config:
...
def testuvg(global_step, testfull=False):
    with torch.no_grad():
        ...
        for batch_idx, input in enumerate(test_loader):
            ...
            # overall add I
            ibpp = torch.mean(ref_bpp).item()
            ipsnr = torch.mean(ref_psnr).item()
            imss = torch.mean(ref_msssim).item()
            ...
            for i in range(seqlen):
                ...
                inputframe, refframe = Var(input_image), Var(ref_image)
#                clipped_recon_image, mse_loss, warploss, interloss, bpp_feature, bpp_z, bpp_mv, bpp = net(inputframe, refframe)
#                pbpp = torch.mean(bpp).item()
#                pps = torch.mean(10 * (torch.log(1. / mse_loss) / np.log(10))).item()
#                pmss = ms_ssim(clipped_recon_image.cpu().detach(), input_image, data_range=1.0, size_average=True).item()
                clipped_recon_image, mse_loss, warploss, interloss, bpp_feature, bpp_z, bpp_mv, bpp = net(inputframe, refframe)
                pbpp = torch.mean(bpp).item()

                # ===== Y-PSNR / Y-MS-SSIM (BT.601) =====
                org = inputframe.detach()              # (B,3,H,W) on GPU, [0,1]
                rec = clipped_recon_image.detach()     # (B,3,H,W) on GPU, [0,1]

                y_org = 0.299 * org[:, 0:1] + 0.587 * org[:, 1:2] + 0.114 * org[:, 2:3]
                y_rec = 0.299 * rec[:, 0:1] + 0.587 * rec[:, 1:2] + 0.114 * rec[:, 2:3]

                mse_y = torch.mean((y_rec - y_org) ** 2)
                pps = (10.0 * torch.log10(1.0 / torch.clamp(mse_y, min=1e-10))).item()
                pmss = ms_ssim(y_rec, y_org, data_range=1.0, size_average=True).item()
# ======================================
        ...
        # avoid zero division
        if cntI > 0:
            logger.info("I-only   : bpp=%.6f psnr=%.6f msssim=%.6f" %
                        (sumI_bpp / cntI, sumI_psnr / cntI, sumI_msssim / cntI))
        if cntP > 0:
            logger.info("P-only   : bpp=%.6f psnr=%.6f msssim=%.6f" %
                        (sumP_bpp / cntP, sumP_psnr / cntP, sumP_msssim / cntP))
        sumbpp /= cnt
        sumpsnr /= cnt
        summsssim /= cnt
        log = "UVGdataset : average bpp : %.6lf, average psnr : %.6lf, average msssim: %.6lf\n" % (sumbpp, sumpsnr, summsssim)

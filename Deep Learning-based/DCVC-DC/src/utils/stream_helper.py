...
def get_state_dict(ckpt_path):
    ...
    # 1) 先拿完整的 model_state_dict（含 buffers）
    ...
    # 2) 如果有 ema_state_dict，用它覆盖参数（保持 EMA 的权重），但不丢 buffers
    if isinstance(ckpt, dict) and "ema_state_dict" in ckpt and isinstance(ckpt["ema_state_dict"], dict):
        ema = ckpt["ema_state_dict"]
        # ema 里可能有封装结构，这里假设是“参数字典”，如果不是再告诉我我帮你适配
        # 只覆盖同名 key

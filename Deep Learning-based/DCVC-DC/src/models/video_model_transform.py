...
class DMC(CompressionModel):
    ...
    def forward_one_frame(self, x, dpb, q_in_ckpt=False, q_index=None, frame_idx=0,stage=1,lmbda=0.01):
        ...
        # 彻底修复: 防止数值不稳定产生 NaN
        scales_hat = torch.clamp(scales_hat, min=1e-9)
        mv_scales_hat = torch.clamp(mv_scales_hat, min=1e-9)
        ...
        scales_hat = 5.55 - torch.nn.functional.softplus(5.55 - scales_hat)  
        scales_hat = torch.exp(scales_hat)
        mv_scales_hat = torch.nn.functional.softplus(mv_scales_hat+2.3)-2.3 
        mv_scales_hat = torch.exp(mv_scales_hat)
        ...
        return {"bpp_mv_y": bpp_mv_y,
                ...
                "bpp_train": bpp_train,
                "distortion": distortion,
                "bit_y": bit * (bpp_y/bpp) if bpp > 0 else 0, # 防止除以0
                "bit_z": bit * (bpp_z/bpp) if bpp > 0 else 0,
                "bit_mv_y": bit * (bpp_mv_y/bpp) if bpp > 0 else 0,
                "bit_mv_z": bit * (bpp_mv_z/bpp) if bpp > 0 else 0,
                }

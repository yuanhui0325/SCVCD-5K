...
#        return y_q_scale_enc, y_q_scale_dec, mv_y_q_scale_enc, mv_y_q_scale_dec
    ...
    def get_q_scales_from_ckpt(ckpt_path):
        ...
        # 关键：从 model_state_dict 取，里面包含 buffers（q_scale 一般是 buffer）
        ...
        def fetch(name):
            if name in sd:
                return sd[name].reshape(-1)
            if ("module." + name) in sd:
                return sd["module." + name].reshape(-1)
            for k, v in sd.items():
                if k.endswith("." + name) or k.endswith(name):
                    return v.reshape(-1)
            raise KeyError(f"Cannot find {name} in model_state_dict")

        y_q_scale_enc = fetch("y_q_scale_enc")
        y_q_scale_dec = fetch("y_q_scale_dec")
        mv_y_q_scale_enc = fetch("mv_y_q_scale_enc")
        mv_y_q_scale_dec = fetch("mv_y_q_scale_dec")
        return y_q_scale_enc, y_q_scale_dec, mv_y_q_scale_enc, mv_y_q_scale_dec

...
class UVGGOPDataset(torch.utils.data.Dataset):
    def __init__(self, root_dir, transform=None, gop_size=12, crop_size=512):
        """
        支持两种结构：
        A) root_dir/SeqName/*.png
        B) root_dir/SeqName/ClipName/*.png   (你的 val/sequences/00001/0001/*.png)

        crop_size:
            验证时对每帧做 center-crop（再 64 对齐），用于避免大分辨率 OOM。
            - crop_size=0 或 None：不裁剪（可能 OOM）
            - crop_size=512/256：推荐
        """
        self.root_dir = root_dir
        self.transform = transform
        self.gop_size = gop_size
        ...
        def list_images(dir_path):
            if not os.path.isdir(dir_path):
                return []
            files = sorted([f for f in os.listdir(dir_path) if is_image_file(f)])
            return files

        def list_leaf_dirs_with_images(root):
            ...
            # 情况A：第一层目录就直接有帧
            leaf = [d for d in level1 if len(list_images(d)) > 0]
            if len(leaf) > 0:
                return leaf
            ...
            for d in level1:
                level2 = sorted([
                    os.path.join(d, dd) for dd in os.listdir(d)
                    if os.path.isdir(os.path.join(d, dd))
                ])
                for dd in level2:
                    if len(list_images(dd)) > 0:
                        leaf.append(dd)
            return leaf

        seq_dirs = list_leaf_dirs_with_images(self.root_dir)

        # 为每个 seq_dir 切成若干 GOP（不足 gop_size 也保留，只要 >=2 帧）
    ...
    def __getitem__(self, idx):
        ...
        item = self.video_sequences[idx]
        seq_dir = item["seq_dir"]
        frame_names = item["frames"]
        ...
        if self.crop_size is not None and self.crop_size > 0:
            ...
            ch = (ch // 64) * 64
            if cw < 64 or ch < 64:
                raise ValueError(f"Too small after 64-align crop: {W}x{H} -> {cw}x{ch}")

            x0 = (W - cw) // 2
            y0 = (H - ch) // 2
            frames = [img.crop((x0, y0, x0 + cw, y0 + ch)) for img in frames]
        # -------------------------------------------------------------------

        if self.transform:
            frames = [self.transform(img) for img in frames]

        return torch.stack(frames)  # [S, C, H, W]
...
def main():
    ...
    parser.add_argument('--uvg_crop_size', type=int, default=512, help='Center crop size for validation (64-aligned). 0 disables crop.')
...
#    )
    test_dataset = UVGGOPDataset(
        root_dir=args.uvg_dir,
        transform=transform,
        gop_size=7,  # 你每个clip 7帧的话建议写7（写12也能跑，但不必要）
        crop_size=None if args.uvg_crop_size == 0 else args.uvg_crop_size
    )
